...
export default class Sketchpad extends React.Component {
    ...
    constructor(props) {
        ...
        this.veretexIDCount = this.state.vertexIDCount;
        this.edgeIDCount = this.state.edgeIDCount;
        ...
        return (
            <div id='padRoot'>
                <div id='pad' onClick={this.drawVertex.bind(this)}>
                    <div id='vertexDisplay' style={{visibility: 'hidden'}}>v = {this.state.vertices.length}</div>
                    <div id='edgeDisplay' style={{visibility: 'hidden'}}>e = {this.state.edges.length}</div>
                </div>
                <button id='clearPad' onClick={this.clearPad.bind(this)}>Clear Pad</button>
            </div>
        );
    }
    ...
    drawVertex(ev) {
        if (!this.mouseOverObj) {
            const vertex = new Vertex(ev.clientX - this.vertexDiameter / 2, ev.clientY - this.vertexDiameter / 2, this.vertexIDCount++, this);
            this.vertices.push(vertex);
            this.vertexCount++;
            document.getElementById('vertexDisplay').innerHTML = 'v = ' + this.vertexCount;
...
class Vertex {
    constructor(x, y, id, sketchPad) {
        ...
        document.getElementById('pad').appendChild(this.vertex);
        ...
        this.vertex.addEventListener('mousedown', function () {
            ...
            v.sketchPad.selectElement(v.sketchPad.selectedVertices, v);
        });
        this.vertex.addEventListener('mouseenter', function () {
            //mouse over object
            v.sketchPad.mouseOverObj = v;
        });
        this.vertex.addEventListener('mouseleave', function () {
            //mouse exit object
            v.sketchPad.mouseOverObj = null;
        });
    }
    ...
    moveVertex(x, y) {
        ...
        //width check
        if (x > 2 * this.sketchPad.selectBorderRadius && x < this.sketchPad.padWidth - this.sketchPad.vertexDiameter / 2 - 2 * this.sketchPad.selectBorderRadius) {
            this.x = x;
        }
        //length check
        if (y > 2 * this.sketchPad.selectBorderRadius && y < this.sketchPad.padLength - this.sketchPad.vertexDiameter / 2 - 2 * this.sketchPad.selectBorderRadius) {
            this.y = y;
        }
    ...
    select() {
        this.vertex.style.border = this.selectBorderRadius + 'px ' + this.sketchPad.selectionColor;
        this.y -= this.sketchPad.selectBorderRadius;
        this.x -= this.sketchPad.selectBorderRadius;
        this.vertex.style.top = this.y + 'px';
        this.vertex.style.left = this.x + 'px';
...
class Edge {
    constructor(vertex1, vertex2, id, sketchPad) {
        ...
        document.getElementById('pad').appendChild(this.edge);
        ...
        if (this.isLoop) {
            ...
            this.edge.style.height = this.sketchPad.loopDiameter + 'px';
            this.edge.style.width = this.sketchPad.loopDiameter + 'px';
            this.edge.id = 'loop';
        } else {
            vertex1.addEdge(this);
        ...
        this.edge.addEventListener('mousedown', function () {
            //select the component
            e.sketchPad.selectElement(e.sketchPad.selectedEdges, e);
        });
        this.edge.addEventListener('mouseenter', function () {
            //mouse over object
            e.sketchPad.mouseOverObj = e;
        });
        this.edge.addEventListener('mouseleave', function () {
            //mouse exit object
            e.sketchPad.mouseOverObj = null;
        });
    }
    ...
    positionEdge() {
        if (this.isLoop) {
            ...
            let x = this.vertex1.x - parseFloat(this.edge.style.width) + this.sketchPad.vertexDiameter / 2;
            let y = this.vertex1.y - parseFloat(this.edge.style.height) + this.sketchPad.vertexDiameter / 2;
            if (this.isSelected) {
                x -= this.sketchPad.selectBorderRadius;
                y -= this.sketchPad.selectBorderRadius;
            }
            ...
            if (this.isArc) {
                ...
                this.arrow.style.left = parseFloat(this.edge.style.width) / 2 - this.sketchPad.arrowSize / 2 + 'px';
                this.arrow.style.top = -(this.sketchPad.arrowSize + this.sketchPad.edgeWidth / 2) + 'px';
            }
        } else {
            //is it an arc?
            ...
            if (this.vertex1.isSelected) {
                x1 += this.sketchPad.selectBorderRadius;
                y1 += this.sketchPad.selectBorderRadius;
            }
            if (this.vertex2.isSelected) {
                x2 += this.sketchPad.selectBorderRadius;
                y2 += this.sketchPad.selectBorderRadius;
            }
            ...
            if (this.isArc) {
                ...
                this.arrow.style.top = height / 2 - this.sketchPad.arrowSize / 2 + 'px';
                this.arrow.style.left = -(this.sketchPad.arrowSize / 2 + this.sketchPad.edgeWidth / 2) + 'px';
            }
            this.edge.style.height = height + 'px';
            this.edge.style.top = y - (height / 2) + (this.sketchPad.vertexDiameter / 2) + 'px';
            this.edge.style.left = x - (this.sketchPad.edgeWidth / 2) + (this.sketchPad.vertexDiameter / 2) + 'px';
    ...
    select() {
        this.edge.style.border = this.sketchPad.selectBorderRadius + 'px ' + this.sketchPad.selectionColor;
        this.isSelected = true;
        this.positionEdge();
    }
    ...
    toggleBridge(isBridge) {
        ...
        if (isBridge) {
            if (!this.isSelected) {
                this.edge.style.border = this.sketchPad.selectBorderRadius + 'px ' + this.sketchPad.bridgeColor;
            }
        }else {
            if (this.isSelected) {
                this.select();
